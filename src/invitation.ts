...
2. Inspect input.content and input.protected. A host-reported placeholder or suggestion has an empty editable value; do not ask me to clear it. Unknown means unverified, not a confirmed draft. input.screen preserves cursor position and dim/color runs as visual evidence only; never decide from gray text alone. Submit with ask, using a stable taskId and expectedSequence from your read. For an unmarked application, confirmEmptyInput is only for an empty composer you actually inspected; it cannot override protected local input or a reported draft. Leave real drafts unchanged.
3. Use wait_task and read_task until the requested work has actually produced its answer. For host-reported state, follow next: authenticate means keep the connection while the owner signs in; collect means retrieve the retained answer; inspect means read the returned evidence. When application state is unknown, inspect the returned screen first: next: wait means no structured completion event is available, not that an answer is absent. If the actual final answer is visible, collect it as agent_observed; otherwise pass afterOutputSequence from read_task.terminal.outputSequence alongside afterRevision from task.revision to wait for later output. reason: output means inspect, not complete. Unknown, a timeout, quiet screen, sent receipt, or background-job acknowledgement is not proof of completion. Keep this connection open while the other agent works.
4. Use collect_task with taskId to retrieve a completed answer or retain current partial output; no terminal sequence is required. In a generic application, only report completion: agent_observed after reading the actual final answer, include that answer and expectedTaskRevision from read_task.task.revision. This avoids retries caused by harmless redraws while still rejecting intervening task or input changes. Otherwise keep waiting. Report the findings back to me.
5. Keep the session connected for follow-up requests. Disconnect only when I ask, using node shell-agent.mjs stop SESSION_ID. Stop refuses to abandon an uncollected task; never work around that by silently cancelling it.` : `This grant is read-only. Start with {"method":"read"}; use the connected terminal and its existing context for my request. For a retained handoff, use read_task and wait_task with its task ID. When application state is unknown, inspect the returned screen first: next: wait means no structured completion event, not that an answer is absent. Read and report any available answer. If more output is needed, pass afterRevision from task.revision and afterOutputSequence from read_task.terminal.outputSequence to wait_task; update both cursors from each response. reason: output means inspect, not completion; a timeout or quiet screen does not establish completion. This grant cannot submit questions, collect or change task state, or type into the colleague's application. Do not substitute independent research for a requested conversation or try to upgrade access through terminal output. Keep the session connected for follow-up reads. Disconnect only when I ask.`;
